...
const Menu = ({ showHeader = true, showFooter = true, }) => {
    ...
    // Use separate coupon states for each menu category
    const [breakfastCouponApplied, setBreakfastCouponApplied] = useState({});
    const [lunchCouponApplied, setLunchCouponApplied] = useState({});
    const [dinnerCouponApplied, setDinnerCouponApplied] = useState({});
    ...
    const handleCouponClick = (menuItem, category) => {
        if (menuItem.coupon_code && menuItem.discount_percentage) {
            let couponAppliedState;
            switch (category) {
                case 'Breakfast':
                    couponAppliedState = { ...breakfastCouponApplied }; // Create a copy to avoid mutating state
                    break;
                case 'Lunch':
                    couponAppliedState = { ...lunchCouponApplied }; // Create a copy to avoid mutating state
                    break;
                case 'Dinner':
                    couponAppliedState = { ...dinnerCouponApplied }; // Create a copy to avoid mutating state
                    break;
                default:
                    break;
            }
            ...
            if (!couponAppliedState[menuItem.id]) {
                const discount = (menuItem.price * menuItem.discount_percentage) / 100;
                // You should update the cart items here
                // addItem(menuItem, menuItem.quantity, { discount });

                couponAppliedState[menuItem.id] = true;
                setCartMetadata({ discount });

                switch (category) {
                    case 'Breakfast':
                        setBreakfastCouponApplied(couponAppliedState);
                        break;
                    case 'Lunch':
                        setLunchCouponApplied(couponAppliedState);
                        break;
                    case 'Dinner':
                        setDinnerCouponApplied(couponAppliedState);
                        break;
                    default:
                        break;
                }
                                                            ...
                                                            <h5 className="d-flex justify-content-between border-bottom pb-2">
                                                                <span>
                                                                    ...
                                                                    {menuItem.coupon_code && menuItem.discount_percentage ? (
                                                                        <div>
                                                                            {breakfastCouponApplied[menuItem.id] ? (
                                                                                ...
                                                                                <button
                                                                                    ...
                                                                                    id='couponBtn'
                                                                                    onClick={() => handleCouponClick(menuItem, 'Breakfast')}
                                                                                >
                                                                                    Apply
                                                                                </button>
                                                                            )}
                                                                            {breakfastCouponApplied[menuItem.id] ? null : (
                                                                                <span className="text-success ms-2" id='couponText'>
                                                        ...
                                                        <div className="w-100 d-flex flex-column text-start ps-4">
                                                            <h5 className="d-flex justify-content-between border-bottom pb-2">
                                                                <span>
                                                                    ...
                                                                    {menuItem.coupon_code && menuItem.discount_percentage ? (
                                                                        <div>
                                                                            {lunchCouponApplied[menuItem.id] ? (
                                                                                <span className="text-success ms-2" id='couponText'>
                                                                                    Coupon Applied
                                                                                </span>
                                                                            ) : (
                                                                                <button
                                                                                    className="btn btn-success rounded"
                                                                                    id='couponBtn'
                                                                                    onClick={() => handleCouponClick(menuItem, 'Lunch')}
                                                                                >
                                                                                    Apply
                                                                                </button>
                                                                            )}
                                                                            {lunchCouponApplied[menuItem.id] ? null : (
                                                                                <span className="text-success ms-2" id='couponText'>
                                                                                    Coupon: {menuItem.coupon_code} ({menuItem.discount_percentage}% off)
                                                                                </span>
                                                                            )}
                                                        ...
                                                        <div className="w-100 d-flex flex-column text-start ps-4">
                                                            <h5 className="d-flex justify-content-between border-bottom pb-2">
                                                                <span>
                                                                    ...
                                                                    {menuItem.coupon_code && menuItem.discount_percentage ? (
                                                                        <div className='mt-1'>
                                                                            {dinnerCouponApplied[menuItem.id] ? (
                                                                                <span className="text-success ms-2" id='couponText'>
                                                                                    Coupon Applied
                                                                                </span>
                                                                            ) : (
                                                                                <button
                                                                                    className="btn btn-success rounded"
                                                                                    id='couponBtn'
                                                                                    onClick={() => handleCouponClick(menuItem, 'Dinner')}
                                                                                >
                                                                                    Apply
                                                                                </button>
                                                                            )}
                                                                            {dinnerCouponApplied[menuItem.id] ? null : (
                                                                                <span className="text-success ms-2" id='couponText'>
                                                                                    Coupon: {menuItem.coupon_code} ({menuItem.discount_percentage}% off)
                                                                                </span>
                                                                            )}
