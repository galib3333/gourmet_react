...
const Cart = () => {
    const { isEmpty, cartTotal, items, updateItemQuantity, removeItem, metadata } = useCart(); /* to use cart */
    // console.log(items)
            ...
            if (couponCode) {
                try {
                    ...
                    if (data && data.discount_percentage) {
                        ...
                        setDiscount(0); // Coupon is not valid or does not provide a discount.
                    }
                } catch (error) {
